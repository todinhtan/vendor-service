...
.use(
	(req, res, next) => {
		...
		Coop.findById(req.body._id, 'members -_id', (err, {members}) => {
			...
			if (!membersVal.includes(vendorId))
				return errorRes(401, 'unauthorized', err, req, res)
			next()
		})
})

// all request below this will check if the request owner has been authenticate as member of an existing group

// member unjoin request
